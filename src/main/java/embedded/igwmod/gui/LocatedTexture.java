...
import embedded.igwmod.TessWrapper;
...
public class LocatedTexture implements IReservedSpace, IWidget{
    ...
    public LocatedTexture(ResourceLocation texture, int x, int y, int width, int height){
        ...
        if(texture.getResourcePath().startsWith("server")) {
            try {
                BufferedImage image = ImageIO.read(new FileInputStream(new File(ModMain.proxy.getSaveLocation() + "\\igwmod\\" + texture.getResourcePath().substring(7))));
    ...
    public LocatedTexture(ResourceLocation texture, int x, int y, double scale){
        ...
        try {
            ...
            if(texture.getResourcePath().startsWith("server")) {
                bufferedimage = ImageIO.read(new FileInputStream(new File(ModMain.proxy.getSaveLocation() + "\\igwmod\\" + texture.getResourcePath().substring(7))));
            } else {
