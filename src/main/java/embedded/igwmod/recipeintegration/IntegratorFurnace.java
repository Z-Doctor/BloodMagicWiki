...
import embedded.igwmod.api.FurnaceRetrievalEvent;
...
public class IntegratorFurnace implements IRecipeIntegrator {
	...
	// public static Map<String, ItemStack> autoMappedFurnaceRecipes = new
	...
	public void onCommandInvoke(String[] arguments, List<IReservedSpace> reservedSpaces,
		...
		if (arguments.length != 3 && arguments.length != 4)
			throw new IllegalArgumentException(
					"Code needs to contain 3 or 4 arguments: x, y, inputstack, outputstack. It now contains "
							+ arguments.length + ".");
		int x;
		try {
		...
		locatedTextures
				.add(new LocatedTexture(TextureSupplier.getTexture(Paths.MOD_ID_WITH_COLON + "textures/GuiFurnace.png"),
						x, y, (int) (82 / GuiWiki.TEXT_SCALE), (int) (54 / GuiWiki.TEXT_SCALE)));
		...
		if (arguments[2].startsWith("key=")) {
			...
			try {
				inputStack = (ItemStack) ClientProxy.RecipeMap.get(resultStackCode).get(0).getRecipe(Map.Entry.class).getKey();
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (inputStack != null) {
		...
		if (inputStack != null) {
			locatedStacks.add(new LocatedStack(inputStack, x + IntegratorCraftingRecipe.STACKS_X_OFFSET,
					y + IntegratorCraftingRecipe.STACKS_Y_OFFSET));
		}
		if (resultStack != null) {
			locatedStacks.add(new LocatedStack(resultStack, x + IntegratorCraftingRecipe.STACKS_X_OFFSET + 60,
					y + IntegratorCraftingRecipe.STACKS_Y_OFFSET + 18));
		}
	}
