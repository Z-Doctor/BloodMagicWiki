...
public class IntegratorBloodOrbCraftingRecipe extends BaseIntegratorRecipe {
	...
	public void addAutomaticCraftingRecipe(String code, List<LocatedStack> locatedStacks, List<IWidget> locatedTextures,
		...
		IRecipe mappedRecipe = null;
		// if(ClientProxy.RecipeMap.containsKey(key) &&
		// ClientProxy.RecipeMap.get(key) instanceof IRecipe)
		try {
			mappedRecipe = ClientProxy.RecipeMap.get(key).get(0).getRecipe(IRecipe.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (mappedRecipe != null) {
			if (mappedRecipe instanceof ShapedBloodOrbRecipe) {
				...
				// locatedStacks.add(new LocatedStack(recipe.func_77571_b(), x +
				// RESULT_STACK_X_OFFSET,
				// y + RESULT_STACK_Y_OFFSET));
				locatedStacks.add(new LocatedStack(recipe.getRecipeOutput(), x + RESULT_STACK_X_OFFSET,
						y + RESULT_STACK_Y_OFFSET));
				locatedStrings.add(new LocatedString(I18n.format(ModMain.MODID + ".gui.crafting.shapedorb"),
						x * 2 + 120, y * 2, 0xFF000000, false));
				locatedStrings.add(
						new LocatedString("Tier: " + recipe.getTier(), x * 2 + 120, y * 2 + 10, 0xFF000000, false));
			} else if (mappedRecipe instanceof ShapelessBloodOrbRecipe) {
				ShapelessBloodOrbRecipe recipe = (ShapelessBloodOrbRecipe) mappedRecipe;
				for (int i = 0; i < 3; i++) {
					for (int j = 0; j < 3; j++) {
						if (i * 3 + j < recipe.getRecipeSize()) {
							Object obj = recipe.getInput().get(i * 3 + j);
				...
				locatedStacks.add(new LocatedStack(recipe.getRecipeOutput(), x + RESULT_STACK_X_OFFSET,
						y + RESULT_STACK_Y_OFFSET));
				locatedStrings.add(new LocatedString(I18n.format(ModMain.MODID + ".gui.crafting.shapelessorb"),
						x * 2 + 120, y * 2, 0xFF000000, false));
				locatedStrings.add(
						new LocatedString("Tier: " + recipe.getTier(), x * 2 + 120, y * 2 + 10, 0xFF000000, false));
			} else
